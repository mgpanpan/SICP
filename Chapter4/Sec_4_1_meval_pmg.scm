...
(define primitive-procedures
  (list (list 'car car)
        ...
        ;; added for support load, is this right????
        (list 'load (lambda (file-name)
                      (define input-file (open-input-file file-name))
                      (define (loop)
                        (define expression (read input-file))
...
;;                    (eval (read (open-input-file file-name))
;;                           the-global-environment)))

        ;; this two ways of implementation of map have the same problem.
        ;; (list 'map (lambda (f lst)
        ;;             (if (null? lst)
        ;;                 '()
        ;;                 (cons (f (car lst)) (map f (cdr lst))))))
        ;; (list 'map map)
        (list 'test-binary-op (lambda (x y) (+ (* x x) (* y y))))))
